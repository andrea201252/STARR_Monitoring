...
def make_points_gdf(df):
    ...
    if len(df_u) == 0:
        raise RuntimeError("Dataset vuoto dopo rimozione duplicati coordinate.")
...
def build_reference_area(points_gdf, support_gdf, meta=None):
    ...
    n_in = int(sum(1 for p in points_gdf.geometry if geom.contains(p) or geom.intersects(p)))
