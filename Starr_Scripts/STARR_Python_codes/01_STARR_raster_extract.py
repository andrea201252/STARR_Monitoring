...
def spatially_stratified_sample(df):
    ...
    idx = (work.groupby(["_gx","_gy"], sort=False)
...
def run_extraction(base_dirs=None, output_dir=None, verbose=True,
    ...
    report = {
        **{k: v for k, v in meta.items() if k != "raster_transform"},
        ...
        "ratio_donor_project": round(len(donor_df) / max(len(proj_df), 1), 2),
        "meets_3x_guideline": len(donor_df) >= 3 * len(proj_df),
        "raster_transform": tr_dict,
        "pa_filter_audit": {
    ...
    if verbose:
        ...
        print(f"  Ratio donor/proj : {report['ratio_donor_project']:.1f}×  "
              f"({'✓ ≥3×' if report['meets_3x_guideline'] else '✗ <3×'})")
